            ...
            if let topVC = vcAsNavController.topViewController {
                if self.topmostViewController() is ConversationViewController {
                    if let conversationVC = topVC as? ConversationViewController {
                        Task {
                            if let conversationObjectIDToResume = await ConversationResumingManager.getConversationObjectID() {
                                // Ensure conversationObjectID points to a valid object
                                if await ConversationCDHelper.isValidObject(conversationObjectID: conversationObjectIDToResume) {
                                    // Push with conversationObjectID
                                    conversationVC.pushWith(conversationObjectID: conversationObjectIDToResume, animated: true)
                                }
                            }
                        }
