...
class CDHelper {
    ...
    static func appendEssay(prompt: String, essay: String, date: Date, userEdited: Bool, essayArray: inout [Essay], success: @escaping() -> Void) {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
        let managedContext = appDelegate.persistentContainer.viewContext
        let entity = NSEntityDescription.entity(forEntityName: Constants.coreDataEssayEntityName, in: managedContext)!
        let essayObject = Essay(entity: entity, insertInto: managedContext)//NSManagedObject(entity: entity, insertInto: managedContext)
    ...
    static func getAllEssays(essayArray: inout [Essay]) {
        ...
        do {
            essayArray = try managedContext.fetch(fetchRequest) as! [Essay]
        } catch let error as NSError {
            print("Error fetching Essay... \(error), \(error.userInfo)")
        }
    }
    
    static func deleteEssay(id: Int, essayArray: inout [Essay], success: @escaping() -> Void) {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
        let managedContext = appDelegate.persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: Constants.coreDataEssayEntityName)
        ...
        do {
            let fetched = try managedContext.fetch(fetchRequest) as! [Essay]
    ...
    static func updateEssay(id: Int, newEssay: String, userEdited: Bool, essayArray: inout [Essay]) {
        guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
        let managedContext = appDelegate.persistentContainer.viewContext
        let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: Constants.coreDataEssayEntityName)
        fetchRequest.predicate = NSPredicate(format: "id = \(id)")
