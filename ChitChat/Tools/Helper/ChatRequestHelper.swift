...
class ChatRequestHelper: Any {
     ...
     */
    static func get(inputText: String, conversationID: Int?, completion: @escaping (_ outputText: String, _ finishReason: String, _ conversationID: Int?, _ remaining: Int)->Void) {
        AuthHelper.ensure(completion: {authToken in
            let getChatRequest = GetChatRequest(authToken: authToken, inputText: inputText, conversationID: conversationID)
            HTTPSConnector.getChat(request: getChatRequest, completion: {getChatResponse in
                ...
                completion(getChatResponse.body.outputText, finishReason, getChatResponse.body.conversationID, getChatResponse.body.remaining)
            })
        })
    }
