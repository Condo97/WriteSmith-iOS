...
//    }
    ...
    static func getAllChats() -> [ChatObjectLegacy] {
        guard let chatStorageData = UserDefaults.standard.data(forKey: Constants.chatStorageUserDefaultKey) else {
            return []
        }

        guard let chatStorage = try? JSONDecoder().decode([ChatObjectLegacy].self, from: chatStorageData) else {
            return []
        }
