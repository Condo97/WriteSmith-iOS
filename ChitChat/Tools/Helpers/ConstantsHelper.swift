...
class ConstantsHelper {
    ...
    static func update() {
        HTTPSConnector.getImportantConstants(completion: {response in
            ...
            UserDefaults.standard.set(response.body.weeklyDisplayPrice, forKey: Constants.defaultWeeklyDisplayPrice)
            UserDefaults.standard.set(response.body.monthlyDisplayPrice, forKey: Constants.defaultMonthlyDisplayPrice)
        })
    }
